const selectDamagedPallet = ({ damagedPalletDb }) => {
    return async function select(info) {
      ...
      if (id) {
        // select one
        const res = await damagedPalletDb.selectOne({ id });
        ...
        if (res.rowCount > 0) {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            data.push({
              ...
              qty_tbr_pallet: e.qty_tbr_pallet ? e.qty_tbr_pallet : null,
        ...
        // select all
        const res = await damagedPalletDb.exportAll({ from, to });
        ...
        // select all
        const res = await damagedPalletDb.selectAll({});
  ...
  module.exports = selectDamagedPallet;
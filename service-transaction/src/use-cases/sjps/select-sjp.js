const selectSjp = ({ sjpDb }) => {
    return async function select(info) {
      ...
      if (id) {
        ...
        if (res.rowCount > 0) {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            data.push({
              ...
              departureCode: e.depart_code ? e.depart_code : null,
        ...
        if (truck) {
          console.log("truck dapet nih anjay")
          // select all
          const res = await sjpDb.searchTruck({from, to, truck});
          if (res.rowCount > 0) {
            // only when there is data returned 
            const items = res.rows;
            for (let i = 0; i < items.length; i++) {
              ...
              data.push({
                ...
                departureCode: e.depart_code ? e.depart_code : null,
                destination: e.destination_company ? e.destination_company : null,
                destinationCode: e.destination_code ? e.destination_code : null,
                transporter: e.transporter_company ? e.transporter_company : null,
                transporterCode: e.transporter_code ? e.transporter_code : null,
                licensePlate: e.license_plate ? e.license_plate : null,
                driverName: e.driver_name ? e.driver_name : null,
                eta: e.eta ? e.eta : null,
                departureTime: e.departure_time ? e.departure_time : null,

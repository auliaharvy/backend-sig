const query = ({ connects, models }) => {
  ...
  async function deleteItem({ id }) {
    try {
      ...
      const BerMissingPallet = models.BerMissingPallets;
      const res = await BerMissingPallet.update(
        {
          is_deleted: 1,
        },
        {
  ...
  async function updatePallet({ data }) {
    try {
      const pool = await connects();
      ...
      for (const mstPallet of mstPallets) {
        ...
        if (data.transaction_type == 0) {
          if (mstPallet.name == "BER Pallet") {
            dataPalletCompany.quantityNew = parseInt(dataPalletCompany.quantity) + parseInt(data.qty_ber_missing_pallet);
          }
  ...
  async function insertNew({ data }) {
    try {
      // use sequelize on inserting
      const BerMissingPallet = models.BerMissingPallets;
      const res = await BerMissingPallet.create(data);
  ...
  async function getTrxNumber() {
    try {
      ...
      const res = await new Promise((resolve) => {
        ...
        const params = ["BMP", formatedMonth, year];
        pool.query(sql, params, (err, res) => {
          if (err) resolve(err);
          resolve(res);
        });
    ...
    try {
      ...
      const res = await new Promise((resolve) => {
        const sql = `SELECT a.*, b.name as company_name, c.username as reporter_name
          FROM "trx_ber_missing_pallet" as a
